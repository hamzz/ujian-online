...
export const createAdminService = ({ db }: { db: DatabaseContext }) => {
  ...
  const upsertSchoolProfile = async (payload: {
    name: string;
    tagline?: string;
    logo_url?: string;
    banner_url?: string;
    ...
    const existing = await db.query<any>('SELECT id FROM school_profile WHERE id = 1');
  ...
  const getRegistrationSettings = async () => {
    const rows = await db.query<any>('SELECT registration_settings FROM app_settings WHERE id = 1');
  ...
  const updateRegistrationSettings = async (settings: { enabled: boolean; allowed_roles: Role[] }) => {
    const existing = await db.query<any>('SELECT id FROM app_settings WHERE id = 1');
    if (existing.length === 0) {
      await db.query('INSERT INTO app_settings (id, registration_settings) VALUES (1, ?)', [
        JSON.stringify(settings)
      ]);
    } else {
      await db.query('UPDATE app_settings SET registration_settings = ? WHERE id = 1', [
        JSON.stringify(settings)
      ]);
    }
    return settings;
  ...
  const listUsers = async (page: number, pageSize: number) => {
    ...
    const [countRows, users] = await Promise.all([
      db.query<any>('SELECT COUNT(*) as total FROM users'),
      db.query<any>(
  ...
  const createUser = async (payload: {
    ...
    await db.query(
      ...
      [id, payload.username, payload.email ?? null, passwordHash, payload.role, JSON.stringify(payload.profile_data ?? {})]
    );
  ...
  const updateUser = async (
    id: string,
    payload: { username?: string; email?: string; role?: Role; profile_data?: Record<string, unknown> }
  ) =>
    db.query(
      ...
      [
        ...
        payload.email ?? null,
        payload.role ?? null,
        payload.profile_data ? JSON.stringify(payload.profile_data) : null,
        id
      ]
    );

  const deleteUser = (id: string) => db.query('DELETE FROM users WHERE id = ?', [id]);

  const importUsers = async (csv: string) => {
    ...
    for (const row of data) {
      ...
      const existing = await db.query<any>('SELECT id FROM users WHERE username = ?', [username]);
      if (existing.length > 0) {
        skipped += 1;
        continue;
      }
  ...
  const getInsights = async () => {
    ...
    const [userCounts, sessionCount, avgScore, unfinished, activeStudents, activeTeachers, inExam] = await Promise.all([
      db.query<any>('SELECT role, COUNT(*) as total FROM users GROUP BY role'),
      db.query<any>('SELECT COUNT(*) as total FROM exam_sessions'),
      db.query<any>('SELECT AVG(total_score) as avg_score FROM grades'),
      db.query<any>("SELECT COUNT(*) as total FROM exam_sessions WHERE status = 'in_progress'"),
      db.query<any>(activeStudentsSql),
      db.query<any>(activeTeachersSql),
      db.query<any>(
    ...
    const topStudents = await db.query<any>(
    ...
    const lowStudents = await db.query<any>(
      `SELECT u.username, AVG(g.total_score) as avg_score, COUNT(*) as attempts
       FROM grades g
       JOIN exam_sessions s ON s.id = g.session_id
       JOIN users u ON u.id = s.user_id
